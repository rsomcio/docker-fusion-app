...
function () {
  ...
  _proto.isResolved = function isResolved(Component$$1, effectId, effectPromiseThunk) {
    var seenEffectIds = this.seen.get(Component$$1);
    var pendingPromises = this.pending.get(Component$$1); // Initialize if not present
 ...
 * 
 */
var FusionContext = createContext({});
var ServiceContext = createContext(function () {
...
function useService(token) {
  var getService = useContext(ServiceContext);
...
function ServiceConsumer(_ref) {
  ...
  return createElement(ServiceContext.Consumer, null, function (getService) {
    var provides = getService(token);
...
function withServices(deps, mapServicesToProps) {
  ...
  return function (Component$$1) {
    return function WithServices(props) {
      return createElement(ServiceContext.Consumer, null, function (getService) {
        return createElement(Component$$1, _extends({}, resolve(getService), props));
      });
    };
  };
}
...
/* eslint-env browser */
var SkipPrepareToken = createToken('SkipPrepareToken');
